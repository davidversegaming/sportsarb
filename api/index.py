...
def calculate_arbitrage(outcomes: Dict[str, Dict[str, dict]]) -> float:
    """
    Calculate if there's an arbitrage opportunity between different sportsbooks
    Returns: profit_percentage
    """
    ...
    for value, books_odds in outcomes_by_value.items():
        ...
        if total_probability < 1:
            ...
            if profit_percentage > best_arbitrage:
                best_arbitrage = profit_percentage
        ...
        if isinstance(data, list):
            for market in data:
                if (market.get("BettingMarketType") == "Player Prop" and 
                    ...
                    profit_percentage = calculate_arbitrage(odds_for_arbitrage)
